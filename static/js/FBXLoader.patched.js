	...
	class AnimationParser {
		...
		parseClips() {
			...
			try {
				...
				console.warn( 'THREE.FBXLoader: Animation parsing failed, skipping animations.', e.message );
				return [];
			}
		...
		parseAnimationCurves( curveNodesMap ) {
			...
			for ( const nodeID in rawCurves ) {
				...
				if ( relationships !== undefined ) {
					...
					if ( ! curveNode || ! curveNode.curves ) continue;
					...
					if ( animationCurveRelationship.match( /X/ ) ) {
						...
						const morphNode = curveNodesMap.get( animationCurveID );
						if ( morphNode && morphNode.curves ) morphNode.curves[ 'morph' ] = animationCurve;
