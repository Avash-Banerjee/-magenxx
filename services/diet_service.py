...
def generate_diet_plan(user_data, indianize=False):
    ...
    macro_split  = get_macro_targets(daily_cal, body_type, goals, weight_kg)
    ...
    # ── Build food pool ──
    food_pool = _build_food_pool(diet_type, indianize)
...
def _build_food_pool(diet_type, indianize):
    """
    ...
    for category, items in FOOD_DB.items():
        ...
        for item in items:
            ...
            if is_veg and "non_veg" in tags and "veg_ovo" not in tags and "veg_lacto" not in tags:
                continue
            # Indian preference boost (not a hard filter — just allows indian items)
...
def _select_meal_items(meal_name, cal_target, protein_target, carb_target,
    ...
    Uses the category priority list and body-type preferences.
    """
    ...
    # Portion adjustment multiplier per body type
...
def _scale_portion(portion_str, scale):
    ...
    if abs(scale - 1.0) < 0.05:
        return portion_str
    if scale < 1.0:
        return f"{portion_str} (slightly reduced — Endomorph portion)"
    if scale > 1.05:
        return f"{portion_str} (slightly larger — Ectomorph portion)"
    return portion_str
